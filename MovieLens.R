...
#+ INTRODUCTION
#'
#' Movies are an important part of our modern-day lives. They entertain, help us escape reality, take us on adventures and make us think. Movies can even change the way we view the world. We used to watch movies in the cinemas, then on our TVs, and nowadays we watch them mainly on various streaming services. This means that we can watch them on demand, and we can pick from thousands of movies at any point in time. But how do we choose what to watch next? 
#'
...
#'
#' The MovieLens datasets and initial code were provided by the course.
#'
library(tidyverse)
library(caret)
...
#' 
edx %>% summarize(average_rating = mean(rating))
...
year %>% group_by(year) %>% summarize(n= n()) %>% arrange(desc(n)) %>% head(., 10)
...
#+ The Simplest Model
#'
#' First, we need to partition the dataset into a train and test set, and make sure they both look at the same movies and have the same user pool.
#'
...
#' We will also define the residual means squared error (RMSE) that we will use to evaluate our models.
#' 
...
#' Factoring in the "movie effect" improves our prediction from an RMSE of about 1.06 to 0.943. How about the genre of the movie? Some genres tend to get more favorable ratings than others.
#' 
#+ The Movie + Genre Effect
#' To evaluate if adding the information about genre can help improve our prediction, let's add it to our previous model.
...
#' 
#' Next, let's check if adding the year of release improves our prediction.
#' 
...
#' The model we have may still suffer from biases. For example, our best and worst reviewed movies seem to be really obscure movies with few ratings. 
#' 
best_rated <- train_set %>% group_by(movieId, title) %>%
  summarize(avg_rating = mean(rating)) %>% arrange(desc(avg_rating))
head(best_rated, n=10)
tail(best_rated, n=10)

#' We need to account for this by constraining the total variability of the effect sizes. We can do that by penalizing larger or smaller estimates that come from small sample sizes. We add a tuning parameter lambda, that shrinks the estimates that are outliers.

lambdas <- seq(0, 10, 0.25)
rgl <- sapply(lambdas, function(l){
...
#' We can see that regularization improves the results slightly, from  RMSE of about 0.866 to 0.865.
#'
...
movie_effect <- edx %>%
  group_by(movieId) %>%
  summarize(movie_effect = sum(rating - avg)/(n()+lambda))
user_effect <- edx %>% 
  left_join(movie_effect, by="movieId") %>%
  group_by(userId) %>%
  summarize(user_effect = sum(rating - movie_effect - avg)/(n()+lambda))
...
final_test_result <- tibble(method = "Regularized Movie + User Effect Model", RMSE = final_test) 
final_test_result %>% knitr::kable()


#+ CONCLUSION
#' We have explored the MovieLens dataset, looking at the predictive power of different variables, including the movie, user, genre and year of release. The Regularized Movie + User Effect model was the best performing model, with a slight improvement over the second best unregularized Movie + User Effect model. Variables such as genre and year of release did not improve the predictive power of our model, likely because the information carried in these variables was redundant. In the future, better models can likely be achieved by employing more advanced machine learning models.
#'